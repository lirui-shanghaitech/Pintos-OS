...
/* New: project 3: */
#ifndef VM
#define vm_ft_get_page(x, y) palloc_get_page(x)
#define vm_ft_true_free(x) palloc_free_page(x)
#endif
...
process_exit (void)
{
  ...
  printf ("%s: exit(%d)\n", cur->name, cur->ret);
#ifdef VM
  // Delete the supplemental page table
  unmap_all();
  vm_supp_pt_delete(&cur->supplemental_table);
#endif
...
// Lazy load, load them into supp page table, when user them , a page fault happen, the call load_page
#ifdef VM
      vm_supp_pt_set_filesys(&thread_current()->supplemental_table, ofs, page_read_bytes,
        page_zero_bytes, upage, file, writable);
      ofs = ofs + PGSIZE;
#else
      /* Get a page of memory. */
      uint8_t *kpage = vm_ft_get_page (PAL_USER, upage);
      if (kpage == NULL)
        return false;
...
setup_stack (void **esp) 
{
  ...
  kpage = vm_ft_get_page (PAL_USER | PAL_ZERO, PHYS_BASE - PGSIZE);
  if (kpage != NULL) 
    {
...
install_page (void *upage, void *kpage, bool writable)
{
  ...
  bool s1 = (pagedir_get_page (t->pagedir, upage) == NULL)
         && pagedir_set_page (t->pagedir, upage, kpage, writable);
  bool s2 = true;
#ifdef VM
  s2 = vm_supp_pt_set_page(&t->supplemental_table, upage, kpage);
  // When install finished, need to unpin the page
  if (s2&&s1)
  {
    vm_ft_unpin(kpage);
  }
...
unmap_all(void)
{
  ...
  struct list_elem *e;
  for (e = list_begin(&curr -> mmap_table); e != list_end(&curr -> mmap_table); e = list_next(e))
  {
    struct map_elem *map_e = list_entry(e,map_elem, elem);
    ...
    for (size_t offset = 0; offset < file_length(file); offset += PGSIZE) 
    {
      ...
      if (pagedir_is_dirty(curr->pagedir, supp_pt_entry->user_page))
      {
